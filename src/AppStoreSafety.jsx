...
export default function AppStoreSafety() {
  ...
  async function blockUser(profile) {
    ...
    const { error } = await supabase
      .from("blocks")
      .upsert({ blocker_id: userId, blocked_id: profile.id }, { onConflict: "blocker_id,blocked_id" });
    if (!error) {
      await Promise.all([
        supabase.from("friend_requests").delete().eq("sender_id", userId).eq("receiver_id", profile.id),
        supabase.from("friend_requests").delete().eq("sender_id", profile.id).eq("receiver_id", userId),
      ]);
      await loadBlocked();
      setNotice(`${profile.username} is blocked. They can’t friend or DM you.`);
      setResults((prev) => prev.filter((item) => item.id !== profile.id));
    } else {
      setNotice("Couldn’t block this person. Try again.");
    }
    setBusy(false);
  }
  ...
  async function submitSupport(event) {
    event.preventDefault();
    if (!userId || supportMessage.trim().length < 5) return;
    setBusy(true);
    ...
    setBusy(false);
    if (error) {
      setNotice("Couldn’t contact Wavo support. Try again.");
      return;
    }
    setSupportMessage("");
  ...
  async function deleteAccount() {
    ...
    if (error) {
      setBusy(false);
      setNotice("Account deletion failed. Your account was not deleted. Try again or contact support.");
      return;
    }
    ...
    try {
      ...
      // Local cleanup is best effort after the server has deleted the account.
    }
        ...
        <div className="safety-overlay" role="dialog" aria-modal="true" aria-label="Wavo Safety Centre">
          <section className="safety-sheet">
            ...
            <header className="safety-head">
              {screen !== "home" ? (
                <button className="safety-icon-btn" onClick={() => openScreen("home")} aria-label="Back">
                  <ChevronLeft />
                </button>
              ...
              <button className="safety-icon-btn" onClick={() => setOpen(false)} aria-label="Close">
                <X />
              </button>
            </header>
            ...
            {screen === "home" && (
              <div className="safety-home">
                <div className="safety-intro">
                  <ShieldAlert />
                  <div>
                    <h2>You control who reaches you.</h2>
                    <p>Report unsafe behaviour, block someone instantly, contact Wavo, or manage your account.</p>
                  </div>
                </div>
            ...
            {(screen === "report" || screen === "block") && (
              <div className="safety-page">
                ...
                <form className="safety-search" onSubmit={searchPeople}>
                  <Search />
                  <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search username" autoCapitalize="none" autoCorrect="off" />
                ...
                <div className="safety-people-list">
                  {results.map((profile) => (
                    <button key={profile.id} onClick={() => screen === "report" ? setSelected(profile) : blockUser(profile)} disabled={busy}>
                      ...
                      <span className="safety-person-action">{screen === "report" ? "Report" : "Block"}</span>
                    </button>
                  ))}
                </div>
                ...
                {screen === "block" && blocked.length > 0 && (
                  <div className="blocked-section">
                    <span className="safety-eyebrow">BLOCKED</span>
                    {blocked.map((profile) => (
                      <div className="blocked-row" key={profile.id}>
                        <MiniProfile profile={profile} />
                        <button onClick={() => unblockUser(profile)} disabled={busy}>Unblock</button>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {screen === "report" && selected && (
                  <form className="report-card" onSubmit={submitReport}>
                    ...
                    <label>What happened? <span>Optional</span>
                      <textarea value={details} onChange={(e) => setDetails(e.target.value)} maxLength={1500} placeholder="Add details that will help moderation review it" />
                    </label>
                    <div className="report-actions">
                      <button type="button" onClick={() => setSelected(null)}>Cancel</button>
                      <button className="safety-primary" disabled={busy}>Send report</button>
                    </div>
                  </form>
                )}
              </div>
            ...
            {screen === "support" && (
              <form className="safety-page support-page" onSubmit={submitSupport}>
                <span className="safety-eyebrow">CONTACT WAVO</span>
                <h2>Send us a support request.</h2>
                ...
                <label>Message
                  <textarea required minLength={5} maxLength={4000} value={supportMessage} onChange={(e) => setSupportMessage(e.target.value)} placeholder="Tell us what you need help with" />
                </label>
            ...
            {screen === "delete" && (
              <div className="safety-page delete-page">
                <span className="safety-eyebrow danger">DELETE ACCOUNT</span>
                <h2>This permanently deletes your Wavo account.</h2>
                ...
                <label>Type <strong>DELETE</strong> to confirm
                  <input value={deleteText} onChange={(e) => setDeleteText(e.target.value.toUpperCase())} autoCapitalize="characters" autoCorrect="off" />
                </label>
                <button className="delete-account-button" disabled={busy || deleteText !== "DELETE"} onClick={deleteAccount}>
                  <Trash2 /> {busy ? "Deleting…" : "Permanently delete account"}
                </button>
