...
-- Adds in-app support, high-confidence content screening, and a server-only
-- helper used by the delete-account Edge Function.
...
-- Apple guideline 1.2 expects a way to filter objectionable UGC. This is a
-- deliberately narrow server-side filter for high-confidence abusive content;
-- reports and blocks handle context that a word filter cannot safely judge.
...
BEGIN
  IF NOT EXISTS (select 1 from pg_constraint where conname = 'messages_content_safety') THEN
    alter table public.messages
      add constraint messages_content_safety
  ...
  IF NOT EXISTS (select 1 from pg_constraint where conname = 'waves_content_safety') THEN
    alter table public.waves
      add constraint waves_content_safety
      check (private.wavo_content_allowed(body)) not valid;
  END IF;
END
$$;

-- Called only with the service-role key from the delete-account Edge Function.
-- It removes Storage ownership and legacy text-ID content that cannot cascade
-- from auth.users. Normal app data is removed by existing FK cascades.
